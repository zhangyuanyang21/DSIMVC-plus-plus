...
class SafeNetwork(MetaModule):
    def __init__(self, view, input_size, feature_dim, high_feature_dim, class_num):
        ...
        self.label_submodule = nn.Sequential(
            ...
            nn.Softmax(dim=1))
        self.view = view
        self.commonfeature_submodule = nn.Sequential(
            MetaLinear(feature_dim * view, feature_dim * view),
            nn.ReLU(),
            MetaLinear(feature_dim * view, high_feature_dim),
        )
        self.commonlabel_submodule = nn.Sequential(
            MetaLinear(feature_dim * view, feature_dim * view),
            nn.ReLU(),
            MetaLinear(feature_dim * view, class_num),
            nn.Softmax(dim=1)
        )
    ...
    def Attention_mechanism(self, xs, xs_incomplete):
        zs = []
        zs_incomplete = []
        for v in range(self.view):
            x = xs[v]
            ...
            x_ = xs_incomplete[v]
            z_ = self.encoders[v](x_)
            zs_incomplete.append(z_)
        commonZ = torch.cat(zs, 1)
        commonZ,_ = self.extract_layers(commonZ)
        commonh = self.commonfeature_submodule(commonZ)
        commonq = self.commonlabel_submodule(commonZ)
        commonZ_incomplete = torch.cat(zs_incomplete, 1)
        commonZ_incomplete,_ = self.extract_layers(commonZ_incomplete)
        commonh_incomplete = self.commonfeature_submodule(commonZ_incomplete)
        commonq_incomplete = self.commonlabel_submodule(commonZ_incomplete)
        return commonh, commonq, commonh_incomplete, commonq_incomplete
