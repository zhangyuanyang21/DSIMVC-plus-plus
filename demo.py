...
parser.add_argument('--dataset', default='synthetic3d')
...
def bi_level_train(model, criterion, optimizer, class_num, view,
    ...
    for com_batch, incomplete_batch in zip(com_loader, incomplete_loader):
        ...
        commonh, commonq, commonh_incomplete, commonq_incomplete = meta_net.Attention_mechanism(xs, incomplete_xs)

        loss_list = []
        for v in range(view):
            loss_list.append(criterion.forward_feature(com_hs[v], commonh))
            loss_list.append(criterion.forward_label(com_qs[v], commonq))
        ...
        for v in range(view):
            l_f, l_l = criterion.forward_feature2(incomplete_hs[v], commonh_incomplete), criterion.forward_label(incomplete_qs[v], commonq_incomplete)
        ...
        commonh, commonq, _, _ = meta_net.Attention_mechanism(xs, incomplete_xs)
        ...
        commonh, commonq, commonh_incomplete, commonq_incomplete = model.Attention_mechanism(xs, incomplete_xs)
...
def initial(com_dataset, full_loader, criterion, mask, incomplete_ind):
    ...
    for e in range(1, epochs+1):
        for xs, _, _ in loader:
            ...
            commonh, commonq =online_net.Attention_mechanism(xs)
            loss_list = []
            for v in range(view):
                loss_list.append(1 * criterion.forward_feature(hs[v], commonh))
