...
class Network(nn.Module):
    def __init__(self, view, input_size, feature_dim, high_feature_dim, class_num):
        ...
        self.commonfeature_submodule = nn.Sequential(
            nn.Linear(feature_dim*view, feature_dim*view),
            nn.ReLU(),
    ...
    def Attention_mechanism(self, xs):
        zs = []
        for v in range(self.view):
            x = xs[v]
            z = self.encoders[v](x)
        ...
        commonZ,S = self.extract_layers(commonZ)
        commonh = normalize(self.commonfeature_submodule(commonZ), dim=1)
        commonq = self.commonlabel_submodule(commonZ)
        return commonh, commonq
